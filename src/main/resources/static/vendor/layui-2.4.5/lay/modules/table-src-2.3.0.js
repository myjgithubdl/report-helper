    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          if(options.tbodyIsSetField){
              if(typeof content == "object" && content.rowspan > 1){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            ...
            if(item3.minWidth) attr.push('data-minwidth="'+ item3.minWidth +'"'); //单元格最小宽度
            if(typeof item1[field] == "object" && item1[field].rowspan > 1 )  attr.push('rowspan="'+ item1[field].rowspan +'"'); //单元格最小宽度
              return attr.join(' ');
          }() +' class="'+ function(){ //追加样式
            var classNames = [];
  ...
  table.init = function(filter, settings){
    ...
    elemTable.each(function(){
      ...
      var cols = [], options = $.extend({
        ...
        ,tbodyIsSetField:false //静态表格Tbody部门是否含有字段   用于设置数据部分跨行
      ...
      othis.find('tbody>tr').each(function(i1){
        ...
        tr.children('td').each(function(i2, item2){
          var td = $(this) , field = td.data('field') , rowspan=td.attr("rowspan");
          if(field){
            if(options.tbodyIsSetField && rowspan && rowspan > 1){
                return row[field] = {value:td.html(),rowspan:rowspan};
            }else{
