                ...
                layui.each(data, function (i1, item1) {
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { //追加各种属性
                            ...
                            if (item3.minWidth) attr.push('data-minwidth="' + item3.minWidth + '"'); //单元格最小宽度
                            if (typeof item1[field] == 'object' && item1[field].rowspan > 1) attr.push('rowspan="' + item1[field].rowspan + '"');
                            if (typeof item1[field] == 'object' && item1[field].colspan > 1) attr.push('colspan="' + item1[field].colspan + '"');
                            return attr.join(' ');
                        }() + ' class="' + function () { //追加样式
                            var classNames = [];
            ...
            othis.find('tbody>tr').each(function (i1) {
                ...
                tr.children('td').each(function (i2, item2) {
                    var td = $(this), field = td.data('field'),
                        rowspan = td.attr("rowspan") || 0, colspan = td.attr("colspan") || 0;
                    if (field) {
                        return row[field] = {value: td.html(), rowspan: rowspan,colspan:colspan};
                    }
                });

                if(!options.isSetTbodyTdFieldName){
                    //如果未定义字段名
                    layui.each(cols, function (i3, item3) {
                        var td = tr.children('td').eq(i3),
                            rowspan = td.attr("rowspan") || 0 , colspan = td.attr("colspan") || 0;
                        row[item3.field] = {value: td.html(), rowspan: rowspan,colspan:colspan};
                    });
                }
