...
var DsCommon = {
    baseUrl: ReportHelper.ctxPath + '/rest/report/ds/',
    baseConfUrl: ReportHelper.ctxPath + '/rest/report/conf/',
...
var DsMVC = {
            ...
            $('#dbType').combobox({
                onChange: function (newValue, oldValue) {
                    if(newValue){
                        ...
                        console.log(DsMVC.Model.dbTypes)
    ...
    Controller: {
        ...
        save: function () {
            ...
            ReportHelper.utils.debug($('#options').val());
    ...
    Util: {
        ...
        loadConfigItems: function () {
            $.getJSON(DsMVC.URLs.getConfItems.url + "?key=" + DsCommon.keys.dbType, function (result) {
                DsMVC.Util.toMap(DsMVC.Model.dbTypes, result.respData);
            });
            $.getJSON(DsMVC.URLs.getConfItems.url + "?key=" + DsCommon.keys.dbPoolType, function (result) {
                DsMVC.Util.toMap(DsMVC.Model.dbPoolTypes, result.respData);
            });
        },
